...
Gemini DR silently drops the YAML metadata header specified in the Core Brief. This is an **architectural constraint, not a prompt-engineering problem**. Gemini Deep Research's synthesis pipeline does not support structured outputs (confirmed in Google's API docs: *"The Deep Research Agent currently doesn't support structured outputs"*). The pipeline's synthesis stage applies its own formatting model (auto-generated headings, citations, ToC) and performs self-critique passes that restructure content — YAML frontmatter is outside its output vocabulary. Tested 2026-04-29 with three alternative prompt strategies (pre-prompt anchor, continuation seed, visual copy-action framing) on topic `flowerpot-top50`; all three failed identically to the original wrapper instruction. Previously observed 2026-04-17 on topic `mexico-city-urban-reforestation` (v1.3), 2026-04-28 on topic `flowerpot-top50` (v1.5). **Structural counter (v1.5.1):** removed the dead YAML instruction from the Gemini wrapper. Phase 2 consolidation already handles Gemini reports without YAML headers — the operator should manually prepend the YAML block after export if downstream tooling requires it, or Phase 2 should infer metadata from the report content and filename.
